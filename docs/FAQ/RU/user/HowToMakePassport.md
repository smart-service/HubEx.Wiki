...
description: >-
  Данная статья является кратким руководством к созданию паспорта объекта,
  привязки QR-кода к Объекту и шаблону заявки. Паспорт объекта – это данные
  Объекта, которые отражаются в мобильном приложении инженера или заказчика при
  сканировании QR-кода по кнопке Подача заявки. Если шаблон заявки, привязанный
  к Объекту является публичным, то паспорт объекта (неполный паспорт) будет
  доступен при сканировании QR-кода на мобильном устройстве в любом приложении
  со сканером QR-кодов (откроется в браузере на телефоне/планшете).
...
<ol>
    <li>автоматизации ввода новых объектов (оборудования) с помощью мобильного приложения инженера: подробнее в статье
        <a href="./CreatingObjTemplates.html">Создание шаблона оборудования,
    ...
    <li>автоматизации подачи заявок и просмотра паспорта объекта через мобильные приложения инженера и заказчика:
        подробнее в статье <a href="./CreatingTaskTemplates.html">Создание шаблона
    ...
    <li>автоматизации подачи заявок для незарегистрированных в системе пользователей (любой человек с мобильным
        ...
        <a href="./SelfRegister.html">Подача заявок по QR-коду без приложения,
...
<ul>
    ...
    <li>в новом окне браузера откроется окно для создания QR-кода и шаблона заявки. QR-код связывает <strong>Объект</strong> и шаблон
        ...
        оставил контактную информацию или смог зарегистрироваться в системе HubEx. Подробнее читайте в статье: <a
                href="./CreatingTaskTemplates.html">Создание шаблона
            заявки, подача заявки по QR-коду</a>;
    </li>
