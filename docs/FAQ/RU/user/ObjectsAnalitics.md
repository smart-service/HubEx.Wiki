...
<p>Вы можете изменять представление данных по дням, неделям и месяцам. Для этого используйте специальные кнопки ↑
    (увеличение уровня
    детализации), ↓↓ (уменьшение уровня детализации). Кнопки появляются при наведении на правый верхний угол визуализации. </p>
<div>
...
<p>С помощью кнопки Открыть меню визуализации (синяя шестеренка) можно изменить структуру таблицы с плоской на
    иерархчиескую. Это значит,
   ...
   <div>
    ...
    таблицу с плоской структурой. Кнопка появляются при наведении на правый верхний угол таблицы.</p>
...
<p>Детальный отчет по объекту также можно открыть по кнопке вверху отчета по кнопке <Strong>В
    отчет по объекту</Strong> и на открывшейся странице выбрать Объект.</p>
...
<ul>
    ...
    <li>Среднее время закрытия заявки, в часах - среднее время за которое Заявка от создания переходит на стадию
...
<p>На гафике отображается количество поступивших, закрытых и открытых Зпявок за период. Вы можете изменять представление
...
<p>Данная визуализация отображает выбранный показатель и линию тренда по этому показателю. Выбрать показатель можно по
    кнопке Открыть меню визуализации (синяя шестеренка). </p>

<p id="mainuser">Таблица Закрытые заявки в разрезе исполнителей</p>
<p>В таблице отображается количество закрытых Заявок в разрезе исполнителей.</p>

<p id="mainpiecharts">Диаграммы соотношений заявок по типам оборудования, виду работ, критичности</p>
<p>Далее представлены три круговых диаграммы соотношений, которые могут отображать следующие показатели в разрезе Типов
    оборудования, Видов работ, Критичностей:</p>
<ul>
    <li>Закрытые заявки за период;</li>
    <li>Открытые заявки за период;</li>
    <li>Поступившие заявки за период.</li>
</ul>
<p>Выбрать показательможно по кнопке Открыть меню визуализации (синяя шестеренка).</p>


<p id="maintable">Таблица со списком заявок</p>
<p>Таблица со списокм заявок отображается в свернутом виде. Развернуть таблицу можно по нажатию на кнопку Открыть
...
<p>В Паспорте объекта отображается ключевая информация об Объекте (Тип оборудования, Класс оборудования, Участок и т.д.)
