...
description: >-
  Шаблон заявки в HubEx необходим для автоматизации подачи Заявок через
  мобильное приложение исполнителя и заказчика по QR-коду, а также для подачи
  заявок незарегистрированными пользователями. С помощью шаблона заявки можно
  сгенерировать свой QR-код и наклеить его на объект (оборудование).
keywords: >-
  qr-код, шаблон заявки, скачать QR-код, паспорт объекта, экспорт qr-кодов,
...
<p>Также шаблон заявки необходим для настройки интеграции HubEx с электронной почтой. Подробнее читайте в статье: <a
        href="../admin/TicketMail.html">Интеграция по электронной почте, подача заявок по
...
<li><strong>Приглашение</strong> указывается для того, чтобы потенциальный пользователь смог
    ...
    статье: <a
            href="../admin/Roles.html">Настройка
    ...
    приглашения по участкам) или указать все. Подробнее читайте в статье: <a
            href="./SelfRegister.html">Подача заявок по QR-коду без
...
<p>Если у вас уже есть QR-код, наклеенный на объект (оборудование) и неиспользованный ранее в HubEx, то при привязке
    ...
    также сможете подать <strong>Заявку</strong>. Подробно о привязке существующего QR-кода читайте в статье: <a
            href="./CreatingObjTemplates.html#2">Создание шаблона оборудования,
...
<p>Паспорт объекта - это данные объекта (оборудования), которые отражаются в мобильном приложении <strong>HubEx
    ...
    мобильном браузере). Подробнее читайте в статье: <a href="./SelfRegister.html">Подача
...
<ul>
    ...
    <li>Для подачи заявки нажмите кнопку <strong>Подать заявку</strong> и заполните необходимые поля, описание заявки и
        ...
        Подробнее о заполнении полей заявки в мобильном приложении читайте в статье: <a
                href="./CreatingTicket.html#mobticket">Создание заявки</a>.
    </li>
</ul>
...
<p>Подробнее читайте в статье: <a href="../admin/StageType.html">Стадии заявки</a>.</p>
<div>
...
___
### Следующие шаги:
- [Подача заявок по QR-коду без приложения, самостоятельная регистрация нового пользователя](./SelfRegister.html)
- [Общая аналитика](./GeneralAnalytics.html)
