...
<h5 id="tickettype1">1.1. Тип заявки</h5>
...
<h5 id="customeragreement">1.2. Стадии заявки и настройка жизненного цикла</h5>
...
<h5 id="message">1.3. Настройка оповещений</h5>
...
<p id="trigger1">Сначала настроим оповещение для ответственного по закупкам о том, что
...
<p>Письмо по этому триггеру уведомление придет на почтовый адрес пользователя, чья Роль была указана в правиле
    выбора. </p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 60%;"
...
<h5 id="mobile">1.4. Процесс линейного согласования в web-приложении</h5>
<p>Инициатор создает Заявку с типом Закупка ТМЦ.</p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 90%;"
         src="/attachments/images/FAQ/ADMIN/TicketsAgreement/Ticket.jpg"/>
</div>
<p>Переведите Заявку на стадию <strong>Согласование заявки</strong> или дождитесь автоматического перехода (который был
    <a href="#auto">настроен в рамках жизненного цикла</a> для стадии Согласование заявки). </p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 90%;"
         src="/attachments/images/FAQ/ADMIN/TicketsAgreement/Ticket2.jpg"/>
</div>

<p>Ответственный за согласование (с ролью Ответственный по закупкам) при этом получит сообщение на электронную почту в
    формате, который мы указали в настроках <a href="trigger1">триггера уведомлений</a>.</p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 80%;"
         src="/attachments/images/FAQ/ADMIN/TicketsAgreement/Email.jpg"/>
</div>
<p>Ответственный за согласование под своей учетной записью открывает Заявку и переводит ее либо на стадию Согласована,
    либо Не
    согласована. </p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 90%;"
...
<p id="email">По результатам согласования инициатор Заявки получит сообщение на почту.</p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 80%;"
...
<p>Обратите внимание на стадии со звездочкой: Согласована начальником подразделения и Согласована начальником
...
<p>Тоже самое касается и стадий Согласована ответственным по закупкам и Согласована
    ответственным по закупкам*.</p>
...
<h5 id="">2.1. Тип заявки</h5>
<p>в примере мы будем использовать тот же <strong>Тип заявки</strong>, который использовался для примера <a
        href="#tickettype1">линейного согласования</a> - закупки ТМЦ.</p>
<p>Но настройка Жизненного цикла будет отличаться. Для одного Типа заявки может быть настроен только один Жизненный
    цикл.</p>

ОТСЮДА ПАРАЛЛЕЛЬНОЕ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

<h5 id="customeragreement">2.2. Стадии заявки и настройка жизненного цикла</h5>
...
<ul>
    <li>первую стадию назовем
        ...
        после ее создания (со стадии Новая на стадию Согласование заявки). Далее при переходе на эту стадию мы также
        настроим в
        отправку уведомлений ответственным о необходимости согласовать <strong>Заявку</strong>;
    </li>
    <p>
    <div>
        <img style="margin: 0 auto; display: block; max-width: 60%;"
    ...
    <li><strong>Не согласована</strong> - если ответственные отказываются
        подтверждать <strong>Заявку</strong>, то ее переводят на стадию Не согласована с любой из предыдущих пяти
...
<ul>
    ...
    <li id="auto2">в настройках перехода также можно установить флажок <strong>Выполнить автоматический переход</strong> и задать
        время перехода, например, 5 минут.
        Благодаря этой настройке <strong>Заявка</strong> автоматически через 5 минут перейдет на стадию <strong>Согласование
            заявки</strong>.
        Далее при переходе на эту стадию мы также настроим в
    ...
    <li>установите флажок <strong>Предоставить доступ отдельным ролям</strong> и укажите роли: для стадии <strong>Согласована
        ...
        только для ответственных за согласование. Для стадии <strong>Не согласована</strong> укажите обе этих Роли.

        <p>Роли вы можете выбрать другие, в зависимости от уже созданных в вашем
            тенанте Ролей - это могут быть Руководитель, Начальник отдела и т.д. Выбранная Роль должна быть указана в
            карточке пользователей - ответственных, которые будут согласовать ваши Заявки. Подробнее читайте в
            статьях: <a href="https://wiki.hubex.ru/docs/FAQ/RU/admin/Roles.html">Настройка ролей</a>, <a
    ...
    <li>из стадии <strong>Согласована начальником подразделения</strong> настройте переход на стадию <strong>Согласована
    ...
    <li>установите флажок <strong>Предоставить доступ отдельным ролям</strong> и укажите роли: для стадии <strong>Согласована
        ...
        Ответственный по закупкам</strong>. Т.к. на этой ветке перейти на обе этих стадии может только Ответственный по
        закупкам.
...
<li>из стадии <strong>Согласована
...
<li>установите флажок <strong>Предоставить доступ отдельным ролям</strong> и укажите роли: для стадии <strong>Согласована
    ...
    Начальник подразделения</strong>. Т.к. на этой ветке перейти на обе этих стадии может только Начальник
    подразделения.
...
<h5 id="message">2.3. Настройка оповещений</h5>
<p>Для простой схемы согласования можно настроить уведомления на пяти стадиях: </p>
<ul>
    <li>Согласование заявки - на этой стадии можно отправить уведомление двум согласующим о том, что им необходимо
        согласовать Заявку;
    </li>
    <li>Согласована начальником подразделения - на этой стадии можно отправить уведомление о том, что Начальник
        подразделения согласовал Заявку, и теперь Ответственному по закупкам также необходимо принять решение о
        согласовании;
    </li>
    <li>Согласована ответственным по закупкам - на этой стадии можно отправить уведомление о том, что Ответственный по
    ...
    <li><strong>Согласована ответственным по закупкам*</strong> и <strong>Согласована начальником
        подразделения*</strong> - на этой стадии можно отправить инициатору Заявки о том, что оба ответственных
        согласовали Заявку;
    </li>
    <li>Не согласована - на этой стадии можно отправить уведомление также инициатору Заявки об отказе в согласовании и
...
<p>Далее перейдите в меню <strong>Настройки оповещений - Триггеры уведомлений</strong>. Здесь настраивается условия
    ...
    и шаблон сообщения:</p>
<ul>
    <li id="trigger2">в <strong>Названии триггера</strong> напишите, например, <strong>Согласование для двух ответственных
        (email)</strong>;
    </li>
    <li>в поле <strong>Описание сообщения</strong> можно выбрать вариант шаблона, по которому будет составлен <strong>Заголовок
        сообщения</strong> и <strong>Текст сообщения</strong> в специальной разметке. Например, <strong>Уведомление о
...
<p>Письмо по этому триггеру уведомление придет на почтовые адреса пользователей, чьи Роли былы указаны в правилах
    выбора. </p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 60%;"
         src="/attachments/images/FAQ/ADMIN/TicketsAgreement/Email.jpg"/>
...
<p>Далее настроим уведомление для Ответственного по закупкам и Начальника подразделения. Один из них должен всегда
...
<p>Аналогичным образом создается и настраивается триггер для начальника: в настройках укажите стадию - Согласована
...
<p>После согласования обоими согласующими или отклонения согласования инициатор заявки должен получить уведомление о
...
<p>Письмо по этим триггерам уведомлениий придет на почтовый адрес Ответственного по закупкам. Например, об удачном
...
<h5 id="agreement">2.4. Процесс параллельного согласования в web-приложении</h5>
<p>Инициатор создает Заявку с типом Закупка ТМЦ и переводит ее на стадию <strong>Согласование заявки</strong> или
    дожидается автоматического перехода (который был <a href="#auto2">настроен в рамках жизненного цикла</a> для стадии
    Согласование заявки). </p>
...
<p>Оба ответственных за согласование (Ответственный по закупкам и Начальник подразделения) получают сообщение на электронную почту в
...
<p>Далее один из ответственных открывает Заявку (например, первым будет Начальник подразделения) и переводит ее либо на стадию Согласована начальником подразделения,
    либо Не
    согласована. </p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 90%;"
...
<p>В случае, когда первый ответственный согласовал Заявку, второму (Ответственному по закупкам) приходит уведомление, о том, что первый ответственный уже согласовал Заявку и требуется решение второго. </p>
...
<p>Второй ответственный также открывает Заявку (например, вторым будет Ответственный по закупкам) и переводит ее либо на стадию Согласована ответственным по закупкам* (со звездочкой),
    либо Не
    согласована.</p>
<div>
    <img style="margin: 0 auto; display: block; max-width: 90%;"
