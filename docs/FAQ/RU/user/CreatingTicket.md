...
<ul>
<!--<li>Номер заявки присваивается автоматически, но может быть изменен вручную. Максимальная длина номера - 16 символов. Возможность изменять номер вручную настраивается в меню консоли администрирования Администрирование - Роли. 
При автоматической генерации номера Заявки используется шаблон номера, настроить который можно в консоли администрирования в меню Настройки заявки - Тип заявки (параметр Маска номера заявки). Подробнее читайте в статье <a href="https://wiki.hubex.ru/docs/FAQ/RU/admin/TicketType.html">Типы заявок</a>.</li> -->
