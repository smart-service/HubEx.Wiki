...
description: >-
  Процесс подачи Заявок через Telegram (Телеграм) - простой и эффективный способ
  решения задач ваших заказчиков. Подключение Telegram к HubEx реализуется с
  помощью подключения Telegram-бота. Заявки будут отправляться пользователями в
  чат с ботом и создаваться в HubEx.
...
<p>Для настройки интеграции с Telegram перейдите в <a
        href="./HowToEnterTheAdmin.html">консоль администратора</a> в меню
    <strong>Интеграция -
        Приложения
