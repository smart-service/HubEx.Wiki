...
<ul>
    <li><strong>Подключение через пересылку</strong>: данный способ предполагает настройку переадрессации писем для
...
<ul>
    ...
    <ul>
        ...
        <li>по умолчанию шаблон создается неопубликованным и публиковать его для текущей интеграции необзяательно.
        ...
        <li id="customer">поле <strong>Заказчик</strong> не заполнено по умолчанию, так как шаблон является общим для
            ...
            <strong>Заказчика</strong>, то укажите его компанию, которая затем будет заполнена в <strong>Заявке</strong>;</p>
        </li>
        ...
        <li>в поле <strong>Вид работ</strong> по умолчанию будет указан такой <strong>Вид работ</strong> в карточке
            которого установлен
            флажок <strong>Основной вид работ</strong>, он может быть тоько один в системе. <strong>Вид работ</strong> в
    ...
    <li><strong>Шаг 3:</strong> перейдите в разделе <strong>Интеграция - Интеграция по электронной
        ...
        интеграции. Для разных почтовых сервисом настройки могут немного отличаться. Далее рассмотрим их подробнее.
    </li>
    <p>
    <div>
        <img style="margin: 0 auto; display: block; max-width: 100%;"
...
<ul>
    ...
    <li>пересылка по умолчанию создается, как неактивная. Чтобы подключить пересылку писем необходимо вернуться в
        меню почтовго ящика <strong>Пересылка и POP/IMAP</strong> и выбрать пункт <strong>Пересылать копии входящих
    ...
    <li>дополнительно для пересылаемых писем можно настроить фильтрацию. Фильтрация позволит уточнить, какие именно
        письма должны поступать, как <strong>Заявки</strong> в HubEx. Фильтры доступны в меню <strong>Пересылка и
            POP/IMAP</strong> по
        гиперссылке <strong>настройте фильтры</strong>: в парметрах фильтров можно задать отправителей писем (например,
...
<ul>
    ...
    <li>дополнительно для пересылаемых писем можно настроить фильтрацию. Фильтрация позволит уточнить, какие именно
        ...
        в парметрах фильтров можно задать отправителей писем (например,
        адреса конкретных заказчиков, чтобы ограничить поток сообщений, а также указать тему или даже слова, которые
